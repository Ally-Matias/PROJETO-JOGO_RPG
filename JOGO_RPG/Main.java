...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        //Usa construtores especificos pra recriar os playes no estado em que estavam ao salvar
        Save.carregar(); 
        System.out.println(Color.RESET);
        System.out.println("BEM VINDO!\n");

        //Cria a variavel local do player da vez que será administrado pelo menu personagem.
        Player player;

        //Repete o loop de menu principal recebendo o player da vez pelo menu personagem até receber nulo e parar.
        while(true){
        ...
        //Ao encerrar o jogo, usa o método estático para salvar os argumentos necessarios
        //para construir os players com o construtor especifico que o método carregar usa.
        Save.salvar();
    }

    //Todos os menus do jogo são loops infinitos que esperam uma string capaz de se converter ao inteiro desejado.

    public static Player menuPrincipal(){
        Scanner teclado = new Scanner(System.in);
        ArrayList<Player> playersRank = new ArrayList<>(); //Variavel que recebe uma cópia de Personagens.players para ordenar com a interface comparable.
        String nome;
        String opcao;
        String classe;
        ...
        while(true){
            ...
            opcao = teclado.next();
            //O método estático parseCheck é usado para retornar a true or false como uma checagem para saber se a string pode ser convertida.
            if(Utility.parseCheck(opcao)){opcao = opcao;}else{opcao = "6";}
            System.out.println();
            if(Integer.parseInt(opcao) == 0){
                ...
                while(true){
                    ...
                    System.out.print(Color.BLACK_BACKGROUND +"Digite uma opção: "+Color.RESET+"\n");
                    classe = teclado.next();
                    if(Utility.parseCheck(classe)){classe = classe;}else{classe = "5";}
                    System.out.println();
                ...
                //Seleciona qual tipo de instancia de subclasse será atribuida ao Player.
                ...
                if(Personagens.players.size()==0){
                    ...
                    while(true){
                        ...
                        if(Utility.parseCheck(opcPersonagem)){opcPersonagem = opcPersonagem;}else{opcPersonagem = opcPersonagem.valueOf(-1);}
                ...
                if(Personagens.players.size()==0){
                    ...
                    while(true){
                        ...
                        if(number<0 || number>(Personagens.players.size()-1)) {
                            ...
                            //Ao remover o Player de Personagens.players, é reescrito o arquivo de save referente a seu indice.
                            Save.deletar(number);
                            break;
                        }
                    }
                ...
                if(Personagens.players.size()==0){
                    ...
                    while(true){
                        ...
                        //A ordenaçao será feita de acordo com o atributo de pontosExperiencia pois assim foi definido com a Interface comparable
    ...
    //Menu instanciado com a referencia de um Player da lista Personagens.players para que seja manipulado e usado no jogo.
    public static void menuPersonagem(Player player){
        ...
        while(true){
            ...
            System.out.print(Color.RESET);
            opcao = teclado.next();
            if(Utility.parseCheck(opcao)){opcao = opcao;}else{opcao = "5";}
            if(Integer.parseInt(opcao) == 0){
                System.out.println("\n");
                break;
            }else if (Integer.parseInt(opcao) == 1){
                //Cenarios são métodos estáticos encadeados que executam operações e digitalizam textos
                //de acordo com as decisões de input do usuário em pequenos menus
                //O jogo em si é feito com base nesses cenários que oferecem a oportunidade de rejogar e continuar evoluindo.
                while(true){
                    ...
                    opcAlt = teclado.next();
                    System.out.println();
                    if(Utility.parseCheck(opcAlt)){opcAlt = opcAlt;}else{opcAlt = "5";}
                    if(Integer.parseInt(opcAlt) == 0){
                        ...
                        //Cada cenário é único e independe dos outros, a base da sua historia e narrativa é criada por qualquer um que deseje contribuir com o jogo.
                        System.out.print(Color.BLACK_BACKGROUND+"\nAutores: "+Color.PURPLE+"Davi Carioca e Fábio Eduardo"+Color.RESET+"\n");
                        player.curarTotal();
                        System.out.println("\n" + Color.BLACK_BACKGROUND +"Voce pode selecionar um novo cenario para jogar:"+Color.RESET);
                        ...
                        System.out.print(Color.BLACK_BACKGROUND+"\nAutor: "+Color.PURPLE+"Alliquisson Matias"+Color.RESET+"\n");
                        player.curarTotal();
                        System.out.println("\n" + Color.BLACK_BACKGROUND +"Voce pode selecionar um novo cenario para jogar:"+Color.RESET);
                    }else{
                        System.out.println(Color.YELLOW +"\nDigite uma opcao valida!!!\n\n"+ Color.RESET);
